...
class ProductTest extends AbstractTestCase
{
    public function testFindReturnsProduct()
    {
        ...
        $return = $mapper->find(['product_id' => $productId]);
    ...
    {
        $productId = $this->insertProduct();
        $linker = ['category_id' => 1, 'website_id' => 1, 'product_id' => $productId];
    ...
    public function testAddOptionCreatesLinker()
    {
        ...
        $select->where(['product_id' => $productId, 'option_id' => $optionId]);
        $testMapper = $this->getTestMapper();
    ...
    public function testRemoveOptionRemovesLinker()
    {
        ...
        $productId = $this->insertProduct();
        $testMapper = $this->getTestMapper();
        $linker = ['product_id' => $productId, 'option_id' => $optionId];
    ...
    public function testSortOptionsChangesOrderOfOptions()
    {
        ...
        $productId = 1;
        $linker1 = ['product_id' => $productId, 'option_id' => 3, 'sort_weight' => 0];
        $linker2 = ['product_id' => $productId, 'option_id' => 4, 'sort_weight' => 1];
        $testMapper = $this->getTestMapper();
        ...
        $select->where(['option_id' => 4, 'sort_weight' => 0]);
        $result = $testMapper->query($select);
        $this->assertTrue(is_array($result));
    }
